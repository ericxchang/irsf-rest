...
class MobileIdDatasetController extends BaseRestController {
	...
	@RequestMapping(value = "/tosproviders", method = RequestMethod.GET)
	public ResponseEntity<String> getTOSProviderList(@RequestHeader Map<String, String> header) {
		ResponseEntity<String> rv;
		try {
			UserDefinition loginUser = getLoginUser(header);
			assertAuthorized(loginUser, PermissionRole.CustAdmin.value() + "," + PermissionRole.User.value());

			List<RangeNdc> results = mobileIdDataService.findAllTosProviderList();
			rv = makeSuccessResult("", results);
